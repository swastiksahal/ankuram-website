...
A26 — the /ib-pyp-tuition-hyderabad redirect is removed from .htaccess, and the URL is added to sitemap.xml. Approved by Swastik, 20 Sep 2026. Invariant 2 is waived for this one line and this one sitemap entry; nothing else in either file changed.
  WHY: .htaccess carried `RewriteRule ^ib-pyp-tuition-hyderabad/?$ /ib-myp-tuition-hyderabad [L,R=301]`, collateral damage from the bulk IB consolidation on the three lines above it. A full PYP page exists and is healthy — 35487 bytes, H1 "IB PYP (Primary Years Programme) Tuition in Hyderabad - Class 1 to 5", canonical pointing at itself, index,follow, GA4 G-MQRSS8DKLE x2 and Ads AW-10954184691 x2 — and GSC still shows it Tier A with 1 click and 64 impressions. The redirect sent Class 1-5 parents to a Class 6-10 page. It also made the IB PYP chip added in A22/A24 land on the MYP page.
  THE EDIT: exactly one line removed, anchored on the RewriteRule rather than the bare string, because "ib-pyp-tuition-hyderabad" appears twice — line 6 is a comment documenting the redirect and is deliberately KEPT as the record of what was done and undone.
    .htaccess before 6683 bytes -> after 6604 bytes; the line including its newline is 79 bytes. 6683 - 79 = 6604, confirmed on the server.
    grep '^\s*RewriteRule \^ib-pyp-tuition-hyderabad' : 1 before, 0 after.
    grep '^#.*ib-pyp-tuition-hyderabad' : 1 before, 1 after.
  NOTE on the brief's step 4: it asked to confirm the file still ends with a newline. It never did — the last byte was and remains "l". No newline was added, because that would have changed bytes beyond the one line and missed the 6604 target.
  SITEMAP: one line added after the ib-myp entry, matching the neighbours' exact format `  <url><loc>https://ankuramtuition.com/ib-pyp-tuition-hyderabad</loc></url>`, alphabetically placed. 83 -> 84 <loc> entries, still valid XML, no duplicates. robots.txt untouched.
  VERIFIED LIVE: / 200; /diagnostic-assessment 200; /ib-pyp-tuition-hyderabad 200 with zero redirect hops, serving the PYP page; /ib-myp-tuition-hyderabad 200; /ib-landing-page, /study-materials/ib and /ibmyp-year-4-and-5-crash-course all still 301 to ib-myp; /diagnostic-assessment/ still 301 to /diagnostic-assessment; /cbse-class-10/ 200; /online-tuition-class-10-cbse/ 200. The homepage hash is unchanged (8c7ac696), as it was not deployed.
  OUTSTANDING: the CDN serves a stale sitemap.xml. The origin has 84 URLs; every edge node returns the old 83-URL copy under cache-control max-age=2592000, i.e. 30 days, currently age 198322s. A23's query string cannot help here because Google requests the bare URL. A Hostinger cache purge is required for the new sitemap to reach Google.
